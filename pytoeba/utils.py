...
def random_string(length=10, chars=digits+ascii_letters):
...
def salted_sha1(text, salt=None):
    if not salt:
        salt = random_string()
    salted_hash = sha1(text + salt)
...
def get_strategy(strategy='', storage='', *args, **kwargs):
    ...
    if not strategy:
        strategy = settings.SOCIAL_AUTH_STRATEGY

    if not storage:
        storage = settings.SOCIAL_AUTH_STORAGE

    Strategy = import_path(strategy)
    Storage = import_path(storage)

    return Strategy(Storage, *args, **kwargs)


def social_user_pipeline(backend, details, uid, user=None, *args, **kwargs):
    from social.exceptions import AuthAlreadyAssociated

    provider = backend.name
    email = details['email']
    social = backend.strategy.storage.user.get_social_auth(provider, email, uid)
    if social:
...
def get_callback_url(backend_name):
    return reverse_lazy('social_auth_callback', backend_name)
